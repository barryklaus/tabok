...
test('dice redesign preserves existing outcome counts and reuses ready resources',async()=>{
 const{d}=await board();const treasure=d.buildDice('Treasure',0);assert.deepEqual(treasure.userData.labels,['RELIC','ODDITY','KEEPSAKE','BLANK','BLANK','BLANK']);
